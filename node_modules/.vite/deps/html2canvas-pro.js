...
var Html2CanvasConfig = class Html2CanvasConfig2 {
  constructor(options = {}) {
...
var Bounds = (_a = class {
  ...
  add(x, y, w, h) {
    return new _a(this.left + x, this.top + y, this.width + w, this.height + h);
  }
  static fromClientRect(context, clientRect) {
    return new _a(clientRect.left + context.windowBounds.left, clientRect.top + context.windowBounds.top, clientRect.width, clientRect.height);
  }
  static fromDOMRectList(context, domRectList) {
    ...
    return domRect ? new _a(domRect.left + context.windowBounds.left, domRect.top + context.windowBounds.top, domRect.width, domRect.height) : _a.EMPTY;
  }
}, _a.EMPTY = new _a(0, 0, 0, 0), _a);
...
for (i$2 = 0; i$2 < chars$2.length; i$2++) lookup$2[chars$2.charCodeAt(i$2)] = i$2;
var i$2;
var chars$1$1 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
var lookup$1$1 = typeof Uint8Array === "undefined" ? [] : new Uint8Array(256);
for (i$1$1 = 0; i$1$1 < chars$1$1.length; i$1$1++) lookup$1$1[chars$1$1.charCodeAt(i$1$1)] = i$1$1;
var i$1$1;
var decode$1 = function(base642) {
  var bufferLength = base642.length * 0.75, len = base642.length, i2, p = 0, encoded1, encoded2, encoded3, encoded4;
  if (base642[base642.length - 1] === "=") {
    bufferLength--;
    if (base642[base642.length - 2] === "=") bufferLength--;
  }
...
var polyUint16Array$1 = function(buffer) {
  ...
  var bytes = [];
  for (var i2 = 0; i2 < length; i2 += 2) bytes.push(buffer[i2 + 1] << 8 | buffer[i2]);
  return bytes;
};
var polyUint32Array$1 = function(buffer) {
  var length = buffer.length;
  var bytes = [];
  for (var i2 = 0; i2 < length; i2 += 4) bytes.push(buffer[i2 + 3] << 24 | buffer[i2 + 2] << 16 | buffer[i2 + 1] << 8 | buffer[i2]);
  return bytes;
};
...
for (i$3 = 0; i$3 < chars$3.length; i$3++) lookup$3[chars$3.charCodeAt(i$3)] = i$3;
var i$3;
...
var codePointsToCharacterClasses = function(codePoints, lineBreak2) {
  if (lineBreak2 === void 0) lineBreak2 = "strict";
  var types = [];
  var indices = [];
  var categories = [];
  codePoints.forEach(function(codePoint, index) {
    ...
    indices.push(index);
    if (classType === CJ) return types.push(lineBreak2 === "strict" ? NS : ID);
...
var cssFormattedClasses = function(codePoints, options) {
  ...
  var _a7 = codePointsToCharacterClasses(codePoints, options.lineBreak), indicies = _a7[0], classTypes = _a7[1], isLetterNumber = _a7[2];
...
var LineBreaker = function(str, options) {
  ...
  var _a7 = cssFormattedClasses(codePoints, options), indicies = _a7[0], classTypes = _a7[1], forbiddenBreakpoints = _a7[2];
  var length = codePoints.length;
  var lastEnd = 0;
  var nextIndex = 0;
...
var _a2;
var Tokenizer = (_a2 = class {
  static get() {
    return _a2._pool.pop() || new _a2();
  }
  static release(tokenizer) {
    if (_a2._pool.length < _a2.MAX_POOL_SIZE) {
      tokenizer._reset();
      _a2._pool.push(tokenizer);
    }
  }
  constructor() {
    this._value = [];
  ...
  consumeStringToken(endingCodePoint) {
    ...
    let i2 = 0;
    do {
      const codePoint = this._value[i2];
...
}, _a2._pool = [], _a2.MAX_POOL_SIZE = 40, _a2);
...
var getTokenColorValue = (token, i2) => {
  if (token.type === 17) return token.number;
  if (token.type === 16) {
    const max = i2 === 3 ? 1 : 255;
...
var packSrgbLinear = ([r, g, b, a]) => {
  const rgb2 = srgbLinear2rgb([
    r,
    g,
    b
  ]);
  return pack(clamp(Math.round(rgb2[0] * 255), 0, 255), clamp(Math.round(rgb2[1] * 255), 0, 255), clamp(Math.round(rgb2[2] * 255), 0, 255), a);
};
var packXYZ = (args) => {
...
var packLab = (_context, args) => {
  ...
  const [l, a, b, alpha] = extractLabComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(lab2xyz([
    l,
    a,
    b
  ])));
...
var packOkLab = (_context, args) => {
  ...
  const [l, a, b, alpha] = extractLabComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(oklab2xyz([
    l,
    a,
    b
  ])));
...
var packOkLch = (_context, args) => {
  ...
  const [l, c, h, alpha] = extractOkLchComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(oklab2xyz(lch2lab([
    l,
    c,
    h
  ]))));
...
var packLch = (_context, args) => {
  ...
  const [l, c, h, a] = extractLchComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(lab2xyz(lch2lab([
    l,
    c,
    h
  ]))));
...
var packHSL = (context, args) => {
  ...
  const [h, s, l, a] = extractHslComponents(context, args), rgb2 = hsl2rgb([
    h,
    s,
    l
  ]);
  return pack(rgb2[0] * 255, rgb2[1] * 255, rgb2[2] * 255, s === 0 ? 1 : a);
};
...
var hsl2rgb = ([h, s, l]) => {
  ...
  const t2 = l <= 0.5 ? l * (s + 1) : l + s - l * s, t1 = l * 2 - t2;
  return [
    hue2rgb(t1, t2, h + 1 / 3),
    hue2rgb(t1, t2, h),
    hue2rgb(t1, t2, h - 1 / 3)
...
var lab2xyz = (lab) => {
  const fy = (lab[0] + 16) / 116, fx = lab[1] / 500 + fy, fz = fy - lab[2] / 200, k = 24389 / 27, e = 24 / 116, xyz = [
    ...
    (fz > e ? fz ** 3 : (116 * fz - 16) / k) * 0.2958 / 0.3585
  ];
  return d50toD65([
    xyz[0],
    xyz[1],
...
var p32p3Linear = (p3) => {
  return p3.map((c) => {
    const sign = c < 0 ? -1 : 1;
    if (c * sign <= 0.04045) return c / 12.92;
...
var _a3 = 1.09929682680944;
var _b = 0.018053968510807;
var rec20202rec2020Linear = (rgb2) => {
  return rgb2.map(function(c) {
    return c < _b * 4.5 ? c / 4.5 : Math.pow((c + _a3 - 1) / _a3, 1 / 0.45);
  });
};
var rec2020Linear2rec2020 = (rgb2) => {
  return rgb2.map(function(c) {
    return c >= _b ? _a3 * Math.pow(c, 0.45) - (_a3 - 1) : 4.5 * c;
  });
};
...
var _color = (context, args) => {
  ...
  if (!isRelativeTransform(tokens)) {
    ...
    const extractComponent = (color2, token) => {
      ...
      if (isIdentToken(token)) return color2[posFromVal(token.value)];
      ...
      if (token.type === 18) {
        ...
        if (token.name === "calc") return safeEvalArithmetic(parseCalc(args2).replace(/r|x/, color2[0].toString()).replace(/g|y/, color2[1].toString()).replace(/b|z/, color2[2].toString()));
      }
      return null;
    };
...
var CounterState = class {
  ...
  parse(style) {
    ...
    let canReset = true;
    if (counterIncrement2 !== null) counterIncrement2.forEach((entry) => {
    ...
    const counterNames = [];
    if (canReset) counterReset2.forEach((entry) => {
...
var createCounterText = (value, type, appendSuffix) => {
  ...
  switch (type) {
    ...
    case 17:
    case 48:
      return createCJKCounter(value, "零一二三四五六七八九", CHINESE_INFORMAL_MULTIPLIERS, "負", cjkSuffix, 14);
    case 47:
      return createCJKCounter(value, "零壹貳參肆伍陸柒捌玖", CHINESE_FORMAL_MULTIPLIERS, "負", cjkSuffix, 15);
    case 42:
      return createCJKCounter(value, "零一二三四五六七八九", CHINESE_INFORMAL_MULTIPLIERS, "负", cjkSuffix, 14);
    case 41:
      return createCJKCounter(value, "零壹贰叁肆伍陆柒捌玖", CHINESE_FORMAL_MULTIPLIERS, "负", cjkSuffix, 15);
    case 26:
      return createCJKCounter(value, "〇一二三四五六七八九", "十百千万", JAPANESE_NEGATIVE, cjkSuffix, 0);
    case 25:
      return createCJKCounter(value, "零壱弐参四伍六七八九", "拾百千万", JAPANESE_NEGATIVE, cjkSuffix, 7);
    case 31:
      return createCJKCounter(value, "영일이삼사오육칠팔구", "십백천만", KOREAN_NEGATIVE, koreanSuffix, 7);
    case 33:
      return createCJKCounter(value, "零一二三四五六七八九", "十百千萬", KOREAN_NEGATIVE, koreanSuffix, 0);
    case 32:
      return createCJKCounter(value, "零壹貳參四五六七八九", "拾百千", KOREAN_NEGATIVE, koreanSuffix, 7);
    case 18:
...
var linearGradient = (context, tokens) => {
  let angle$2 = deg(180);
  const stops = [];
  parseFunctionArgs(tokens).forEach((arg, i2) => {
    if (i2 === 0) {
    ...
    if (i2 === 0) {
      ...
      isColorStop = arg.reduce((acc, token) => {
        if (isAtPosition) {
          ...
          else if (isLengthPercentage(token) || isLength(token)) position2.push(token);
...
var prefixRadialGradient = (context, tokens) => {
  ...
  parseFunctionArgs(tokens).forEach((arg, i2) => {
    let isColorStop = true;
    if (i2 === 0) isColorStop = arg.reduce((acc, token) => {
      if (isIdentToken(token)) switch (token.value) {
        case "center":
          position2.push(FIFTY_PERCENT);
          return false;
    ...
    else if (i2 === 1) isColorStop = arg.reduce((acc, token) => {
      if (isIdentToken(token)) switch (token.value) {
        case CIRCLE:
          shape = 0;
          return false;
...
var computeLineHeight = (token, fontSize2) => {
  if (isIdentToken(token) && token.value === "normal") return 1.2 * fontSize2;
  else if (token.type === 17) return fontSize2 * token.number;
  else if (isLengthPercentage(token)) return getAbsoluteValue(token, fontSize2);
  return fontSize2;
};
var listStyleImage = {
...
var parseCounterValue = (tokens, defaultNumber) => {
  ...
  for (let i2 = 0; i2 < filtered.length; i2++) {
    const counter = filtered[i2];
    const next = filtered[i2 + 1];
    if (counter.type === 20) {
...
var quotes = {
  ...
  parse: (_context, tokens) => {
    ...
    for (let i2 = 0; i2 < filtered.length; i2 += 2) {
      const open = filtered[i2].value;
      const close = filtered[i2 + 1].value;
      quotes2.push({
        open,
        close
      });
...
var objectPosition = {
  ...
  parse: (_context, tokens) => {
    const result = [];
    let i2 = 0;
    while (i2 < tokens.length && result.length < 2) {
...
var _a4;
var CSSParsedDeclaration = (_a4 = class {
  get border() {
  ...
  constructor(context, declaration) {
    const standardProps = _a4.standardProps;
    if (declaration.display === "none") {
      ...
      const overflowTuple2 = parse(context, overflow, void 0);
      this.overflowX = overflowTuple2[0];
      this.overflowY = overflowTuple2[overflowTuple2.length > 1 ? 1 : 0];
      return;
    }
...
var DocumentCloner = class {
    ...
    try {
      ...
      if (sheet && sheet.cssRules) {
        const css = [].slice.call(sheet.cssRules, 0).reduce((css2, rule) => {
          if (rule && typeof rule.cssText === "string") return css2 + rule.cssText;
          return css2;
        }, "");
  ...
  */
  ...
  resolvePseudoContent(node, clone, style, pseudoElt) {
    ...
    if (!document2 || !value || value === "none" || value === "-moz-alt-content" || style.display === "none") return;
    ...
    declaration.content.forEach((token) => {
      if (token.type === 0) anonymousReplacedElement.appendChild(document2.createTextNode(token.value));
        ...
        if (token.name === "attr") {
          ...
          if (attr.length) anonymousReplacedElement.appendChild(document2.createTextNode(node.getAttribute(attr[0].value) || ""));
...
var copyCSSStyles = (style, target) => {
  const parts = [];
  for (let i2 = style.length - 1; i2 >= 0; i2--) {
    const property = style.item(i2);
...
for (i$1 = 0; i$1 < chars$1.length; i$1++) lookup$1[chars$1.charCodeAt(i$1)] = i$1;
var i$1;
...
var testIOSLineBreak = (document2) => {
  ...
  const textList = toCodePoints$1(node.data).map((i2) => fromCodePoint$1(i2));
  let offset = 0;
  let prev = {};
  const supports = textList.every((text, i2) => {
...
var testForeignObject = (document2) => {
  ...
  canvas.height = size;
  const ctx = canvas.getContext("2d");
  if (!ctx) return Promise.reject(new Error("Failed to get 2D rendering context"));
  ...
  ctx.fillStyle = "red";
  ctx.fillRect(0, 0, size, size);
  return loadSerializedSVG(svg).then((img2) => {
    ...
    return isGreenPixel(data) ? loadSerializedSVG(createForeignObjectSVG(size, size, 0, 0, node)) : Promise.reject(new Error("ForeignObject rendering not supported"));
  }).then((img2) => {
    ctx.drawImage(img2, 0, 0);
...
var equalPath = (a, b) => {
  if (a.length === b.length) return a.some((v, i2) => v === b[i2]);
  return false;
};
...
var BezierCurve = class BezierCurve2 {
...
var BoundCurves = class {
  constructor(element) {
    ...
    this.topLeftBorderDoubleOuterBox = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 / 3, bounds.top + borderTopWidth2 / 3, tlh - borderLeftWidth2 / 3, tlv - borderTopWidth2 / 3, 0) : new Vector(bounds.left + borderLeftWidth2 / 3, bounds.top + borderTopWidth2 / 3);
    this.topRightBorderDoubleOuterBox = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + topWidth, bounds.top + borderTopWidth2 / 3, trh - borderRightWidth2 / 3, trv - borderTopWidth2 / 3, 1) : new Vector(bounds.left + bounds.width - borderRightWidth2 / 3, bounds.top + borderTopWidth2 / 3);
    this.bottomRightBorderDoubleOuterBox = brh > 0 || brv > 0 ? getCurvePoints(bounds.left + bottomWidth, bounds.top + rightHeight, brh - borderRightWidth2 / 3, brv - borderBottomWidth2 / 3, 2) : new Vector(bounds.left + bounds.width - borderRightWidth2 / 3, bounds.top + bounds.height - borderBottomWidth2 / 3);
    this.bottomLeftBorderDoubleOuterBox = blh > 0 || blv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 / 3, bounds.top + leftHeight, blh - borderLeftWidth2 / 3, blv - borderBottomWidth2 / 3, 3) : new Vector(bounds.left + borderLeftWidth2 / 3, bounds.top + bounds.height - borderBottomWidth2 / 3);
    this.topLeftBorderDoubleInnerBox = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 * 2 / 3, bounds.top + borderTopWidth2 * 2 / 3, tlh - borderLeftWidth2 * 2 / 3, tlv - borderTopWidth2 * 2 / 3, 0) : new Vector(bounds.left + borderLeftWidth2 * 2 / 3, bounds.top + borderTopWidth2 * 2 / 3);
    this.topRightBorderDoubleInnerBox = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + topWidth, bounds.top + borderTopWidth2 * 2 / 3, trh - borderRightWidth2 * 2 / 3, trv - borderTopWidth2 * 2 / 3, 1) : new Vector(bounds.left + bounds.width - borderRightWidth2 * 2 / 3, bounds.top + borderTopWidth2 * 2 / 3);
    this.bottomRightBorderDoubleInnerBox = brh > 0 || brv > 0 ? getCurvePoints(bounds.left + bottomWidth, bounds.top + rightHeight, brh - borderRightWidth2 * 2 / 3, brv - borderBottomWidth2 * 2 / 3, 2) : new Vector(bounds.left + bounds.width - borderRightWidth2 * 2 / 3, bounds.top + bounds.height - borderBottomWidth2 * 2 / 3);
    this.bottomLeftBorderDoubleInnerBox = blh > 0 || blv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 * 2 / 3, bounds.top + leftHeight, blh - borderLeftWidth2 * 2 / 3, blv - borderBottomWidth2 * 2 / 3, 3) : new Vector(bounds.left + borderLeftWidth2 * 2 / 3, bounds.top + bounds.height - borderBottomWidth2 * 2 / 3);
    this.topLeftBorderStroke = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 / 2, bounds.top + borderTopWidth2 / 2, tlh - borderLeftWidth2 / 2, tlv - borderTopWidth2 / 2, 0) : new Vector(bounds.left + borderLeftWidth2 / 2, bounds.top + borderTopWidth2 / 2);
    this.topRightBorderStroke = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + topWidth, bounds.top + borderTopWidth2 / 2, trh - borderRightWidth2 / 2, trv - borderTopWidth2 / 2, 1) : new Vector(bounds.left + bounds.width - borderRightWidth2 / 2, bounds.top + borderTopWidth2 / 2);
    this.bottomRightBorderStroke = brh > 0 || brv > 0 ? getCurvePoints(bounds.left + bottomWidth, bounds.top + rightHeight, brh - borderRightWidth2 / 2, brv - borderBottomWidth2 / 2, 2) : new Vector(bounds.left + bounds.width - borderRightWidth2 / 2, bounds.top + bounds.height - borderBottomWidth2 / 2);
    this.bottomLeftBorderStroke = blh > 0 || blv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 / 2, bounds.top + leftHeight, blh - borderLeftWidth2 / 2, blv - borderBottomWidth2 / 2, 3) : new Vector(bounds.left + borderLeftWidth2 / 2, bounds.top + bounds.height - borderBottomWidth2 / 2);
    ...
    this.topLeftPaddingBox = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2, bounds.top + borderTopWidth2, Math.max(0, tlh - borderLeftWidth2), Math.max(0, tlv - borderTopWidth2), 0) : new Vector(bounds.left + borderLeftWidth2, bounds.top + borderTopWidth2);
    this.topRightPaddingBox = trh > 0 || trv > 0 ? getCurvePoints(bounds.left + Math.min(topWidth, bounds.width - borderRightWidth2), bounds.top + borderTopWidth2, topWidth > bounds.width + borderRightWidth2 ? 0 : Math.max(0, trh - borderRightWidth2), Math.max(0, trv - borderTopWidth2), 1) : new Vector(bounds.left + bounds.width - borderRightWidth2, bounds.top + borderTopWidth2);
    this.bottomRightPaddingBox = brh > 0 || brv > 0 ? getCurvePoints(bounds.left + Math.min(bottomWidth, bounds.width - borderLeftWidth2), bounds.top + Math.min(rightHeight, bounds.height - borderBottomWidth2), Math.max(0, brh - borderRightWidth2), Math.max(0, brv - borderBottomWidth2), 2) : new Vector(bounds.left + bounds.width - borderRightWidth2, bounds.top + bounds.height - borderBottomWidth2);
    this.bottomLeftPaddingBox = blh > 0 || blv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2, bounds.top + Math.min(leftHeight, bounds.height - borderBottomWidth2), Math.max(0, blh - borderLeftWidth2), Math.max(0, blv - borderBottomWidth2), 3) : new Vector(bounds.left + borderLeftWidth2, bounds.top + bounds.height - borderBottomWidth2);
    this.topLeftContentBox = tlh > 0 || tlv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 + paddingLeft2, bounds.top + borderTopWidth2 + paddingTop2, Math.max(0, tlh - (borderLeftWidth2 + paddingLeft2)), Math.max(0, tlv - (borderTopWidth2 + paddingTop2)), 0) : new Vector(bounds.left + borderLeftWidth2 + paddingLeft2, bounds.top + borderTopWidth2 + paddingTop2);
    this.topRightContentBox = trh > 0 || trv > 0 ? getCurvePoints(bounds.left + Math.min(topWidth, bounds.width + borderLeftWidth2 + paddingLeft2), bounds.top + borderTopWidth2 + paddingTop2, topWidth > bounds.width + borderLeftWidth2 + paddingLeft2 ? 0 : trh - borderLeftWidth2 + paddingLeft2, trv - (borderTopWidth2 + paddingTop2), 1) : new Vector(bounds.left + bounds.width - (borderRightWidth2 + paddingRight2), bounds.top + borderTopWidth2 + paddingTop2);
    this.bottomRightContentBox = brh > 0 || brv > 0 ? getCurvePoints(bounds.left + Math.min(bottomWidth, bounds.width - (borderLeftWidth2 + paddingLeft2)), bounds.top + Math.min(rightHeight, bounds.height + borderTopWidth2 + paddingTop2), Math.max(0, brh - (borderRightWidth2 + paddingRight2)), brv - (borderBottomWidth2 + paddingBottom2), 2) : new Vector(bounds.left + bounds.width - (borderRightWidth2 + paddingRight2), bounds.top + bounds.height - (borderBottomWidth2 + paddingBottom2));
    this.bottomLeftContentBox = blh > 0 || blv > 0 ? getCurvePoints(bounds.left + borderLeftWidth2 + paddingLeft2, bounds.top + leftHeight, Math.max(0, blh - (borderLeftWidth2 + paddingLeft2)), blv - (borderBottomWidth2 + paddingBottom2), 3) : new Vector(bounds.left + borderLeftWidth2 + paddingLeft2, bounds.top + bounds.height - (borderBottomWidth2 + paddingBottom2));
  }
};
var getCurvePoints = (x, y, r1, r2, position2) => {
...
var BlendEffect = class {
  constructor(mixBlendMode2) {
    this.mixBlendMode = mixBlendMode2;
    this.type = 4;
    this.target = 6;
    this.compositeOperation = MIX_BLEND_MODE_TO_COMPOSITE[mixBlendMode2];
  }
};
var FilterEffect = class FilterEffect2 {
  constructor(filterString) {
    this.type = 5;
    this.target = 6;
    const parsed = FilterEffect2.parseDropShadow(filterString);
  ...
  */
  static parseDropShadow(filterString) {
    ...
    const matches = FilterEffect2.findDropShadows(filterString);
    if (matches.length === 0) return { safeFilterString: filterString };
    const shadow = FilterEffect2.parseDropShadowBody(matches[0].body);
    let result = "";
    let lastEnd = 0;
  ...
  */
  static parseDropShadowBody(body) {
    const tokens = FilterEffect2.tokenizeFilterArgs(body.trim());
    const lengths = [];
    let color2;
    for (const token of tokens) if (FilterEffect2.isCSSLength(token)) lengths.push(parseFloat(token));
    else if (token) color2 = token;
    if (lengths.length < 2 || lengths.length > 3) return;
    return {
      offsetX: lengths[0],
      offsetY: lengths[1],
      blur: lengths[2] ?? 0,
      color: color2 ?? "rgba(0,0,0,1)"
    };
  }
  /**
...
var ElementPaint = class {
  constructor(container, parent) {
    ...
    if (hasOverflowClip(this.container.styles)) {
      ...
      if (equalPath(borderBox, paddingBox2)) this.effects.push(new ClipEffect(borderBox, 6));
      else {
    ...
    while (parent) {
      ...
      if (inFlow || parent.container.styles.position !== 0 || !parent.parent) {
        ...
        if (hasOverflowClip(parent.container.styles)) {
          ...
          if (!equalPath(borderBox, paddingBox2)) effects.unshift(new ClipEffect(paddingBox2, 6));
        }
...
var buildClipPathEffect = (clipPath2, bounds) => {
  ...
  switch (clipPath2.type) {
    ...
    case 2: {
      ...
      let r;
      if (clipPath2.radius === "closest-side") r = Math.min(cx - bLeft, cy - bTop, bLeft + bWidth - cx, bTop + bHeight - cy);
      else if (clipPath2.radius === "farthest-side") r = Math.max(cx - bLeft, cy - bTop, bLeft + bWidth - cx, bTop + bHeight - cy);
      else r = getAbsoluteValue(clipPath2.radius, Math.sqrt(bWidth * bWidth + bHeight * bHeight) / Math.SQRT2);
      return new ClipPathEffect((ctx) => {
        ctx.beginPath();
    ...
    case 4: {
      ...
      return new ClipPathEffect((ctx) => {
        ...
        if (absPoints.length > 0) {
          ...
          for (let i2 = 1; i2 < absPoints.length; i2++) ctx.lineTo(absPoints[i2][0], absPoints[i2][1]);
          ctx.closePath();
        }
        ctx.clip();
      });
...
var parseStackTree = (parent, stackingContext, realStackingContext, listItems) => {
  parent.container.elements.forEach((child) => {
    ...
    const createsStackingContext2 = child.createsStackingContext;
    ...
    if (treatAsRealStackingContext || createsStackingContext2) {
      ...
      if (child.styles.isPositioned() || child.styles.opacity < 1 || child.styles.isTransformed()) {
        ...
        if (order < 0) {
          let index = 0;
          parentStack.negativeZIndex.some((current, i2) => {
          ...
          let index = 0;
          parentStack.positiveZIndex.some((current, i2) => {
            if (order >= current.element.container.styles.zIndex.order) {
              index = i2 + 1;
              return false;
            } else if (index > 0) return true;
...
var calculateBackgroundPositioningArea = (backgroundOrigin2, element) => {
  if (backgroundOrigin2 === 0) return element.bounds;
  if (backgroundOrigin2 === 2) return contentBox(element);
  return paddingBox(element);
};
var calculateBackgroundPaintingArea = (backgroundClip2, element) => {
  if (backgroundClip2 === 0) return element.bounds;
  if (backgroundClip2 === 2) return contentBox(element);
  return paddingBox(element);
};
var calculateBackgroundRendering = (container, index, intrinsicSize) => {
  ...
  const position2 = getAbsoluteValueForTuple(getBackgroundValueForIndex(container.styles.backgroundPosition, index), backgroundPositioningArea.width - sizeWidth, backgroundPositioningArea.height - sizeHeight);
  const path = calculateBackgroundRepeatPath(getBackgroundValueForIndex(container.styles.backgroundRepeat, index), position2, backgroundImageSize, backgroundPositioningArea, backgroundPaintingArea);
  const offsetX = Math.round(backgroundPositioningArea.left + position2[0]);
  const offsetY = Math.round(backgroundPositioningArea.top + position2[1]);
...
var calculateBackgroundSize = (size, [intrinsicWidth, intrinsicHeight, intrinsicProportion], bounds) => {
  ...
  if (hasIntrinsicProportion) {
    let width2 = 0;
    let height2 = 0;
    if (isLengthPercentage(first)) width2 = getAbsoluteValue(first, bounds.width);
    else if (isLengthPercentage(second)) height2 = getAbsoluteValue(second, bounds.height);
    if (isAuto(first)) width2 = height2 * intrinsicProportion;
    else if (!second || isAuto(second)) height2 = width2 / intrinsicProportion;
...
var TextDecorationRenderer = class {
  ...
  render(bounds, styles) {
    ...
    else if (styles.textDecorationThickness === "from-font") thickness = Math.max(1, Math.floor(styles.fontSize.number * 0.05));
...
var fixIOSSystemFonts = (fontFamilies) => {
  ...
  if (iosVersion !== null && iosVersion >= 15 && iosVersion < 17) return fontFamilies.map((fontFamily2) => iOSBrokenFonts.indexOf(fontFamily2) !== -1 ? `-apple-system, "Helvetica Neue", Arial, sans-serif` : fontFamily2);
  return fontFamilies;
};
...
var TextRenderer = class {
  ...
  */
  iterateLettersWithLetterSpacing(text, letterSpacing2, baseline, writingMode2, renderFn) {
    ...
    for (const letter of letters) {
      ...
      left += this.cachedMeasureText(letter, fontString) + letterSpacing2;
    }
  }
  iterateVerticalGlyphs(text, letterSpacing2, baseline, writingMode2, renderFn) {
    const letters = segmentGraphemes(text.text);
    const fontString = this.ctx.font;
    const rotationAngle = writingMode2 === 4 ? -Math.PI / 2 : Math.PI / 2;
    let top = text.bounds.top;
    for (let i2 = 0; i2 < letters.length; ) {
      ...
      if (!(isSidewaysWritingMode(writingMode2) || !hasCJKCharacters(letter) && letter.trim().length > 0)) {
        ...
        this.ctx.textBaseline = savedBaseline;
        top += this.cachedMeasureText(letter, fontString) + letterSpacing2;
        i2++;
        continue;
      }
      const runStart = i2;
      while (i2 < letters.length) {
        const ch = letters[i2];
        if (!isSidewaysWritingMode(writingMode2) && !hasCJKCharacters(ch) && ch.trim().length === 0) break;
        if (isSidewaysWritingMode(writingMode2) || !hasCJKCharacters(ch) && ch.trim().length > 0) i2++;
        else break;
      }
      this.ctx.save();
      ...
      let runOffset = 0;
      for (let j = runStart; j < i2; j++) {
        renderFn(letters[j], 0, runOffset);
        runOffset += this.cachedMeasureText(letters[j], fontString) + letterSpacing2;
      }
      this.ctx.restore();
      top += runOffset;
    }
  ...
  */
  renderTextWithLetterSpacing(text, letterSpacing2, baseline, writingMode2 = 0) {
    this.renderFillText(text, letterSpacing2, baseline, writingMode2);
  }
  canRenderWholeText(letterSpacing2, writingMode2) {
    return letterSpacing2 === 0 && !isVerticalWritingMode(writingMode2);
  }
  renderFillText(text, letterSpacing2, baseline, writingMode2) {
    if (this.canRenderWholeText(letterSpacing2, writingMode2)) this.ctx.fillText(text.text, text.bounds.left, text.bounds.top + baseline);
    else this.iterateLettersWithLetterSpacing(text, letterSpacing2, baseline, writingMode2, (letter, x, y) => {
      this.ctx.fillText(letter, x, y);
    });
  }
  renderStrokeText(text, letterSpacing2, baseline, writingMode2) {
    if (this.canRenderWholeText(letterSpacing2, writingMode2)) this.ctx.strokeText(text.text, text.bounds.left, text.bounds.top + baseline);
    else this.iterateLettersWithLetterSpacing(text, letterSpacing2, baseline, writingMode2, (letter, x, y) => {
  ...
  */
  renderLineClampedText(text, styles, paintOrder2, baseline, containerBounds) {
    ...
    for (let i2 = 0; i2 < maxLines - 1; i2++) lines[i2].forEach((tb) => this.renderTextBoundWithPaintOrder(tb, styles, paintOrder2, baseline));
    ...
    if (lastLine?.length && containerBounds) {
      ...
      for (const layer of paintOrder2) if (layer === 0) {
        this.ctx.fillStyle = asString(styles.color);
  ...
  */
  renderEllipsisText(text, styles, paintOrder2, baseline, containerBounds) {
    const lineHeight2 = styles.fontSize.number * 1.5;
    const firstTop = text.textBounds[0].bounds.top;
    if (!text.textBounds.every((tb) => Math.abs(tb.bounds.top - firstTop) < lineHeight2 * 0.5)) return false;
    ...
    for (const layer of paintOrder2) if (layer === 0) this.renderTextFillWithShadows(bounds, styles, baseline);
    else if (layer === 1) this.renderTextStrokeWithStyle(bounds, styles, baseline);
    return true;
  }
  async renderTextNode(text, styles, containerBounds) {
    ...
    if (styles.webkitLineClamp > 0 && (styles.display & 2) !== 0 && styles.overflowY === 1 && text.textBounds.length > 0) {
      if (this.renderLineClampedText(text, styles, paintOrder2, baseline, containerBounds)) return;
    }
    if (styles.textOverflow === 1 && containerBounds && styles.overflowX === 1 && text.textBounds.length > 0 && this.renderEllipsisText(text, styles, paintOrder2, baseline, containerBounds)) return;
    text.textBounds.forEach((tb) => {
      paintOrder2.forEach((layer) => {
        if (layer === 0) {
...
var BackgroundRenderer = class {
  ...
  */
  async renderBackgroundImage(container) {
    ...
    for (const backgroundImage2 of container.styles.backgroundImage.slice(0).reverse()) {
      ...
      if (backgroundImage2.type === 0) await this.renderBackgroundURLImage(container, backgroundImage2, index);
      else if (isLinearGradient(backgroundImage2)) this.renderLinearGradient(container, backgroundImage2, index);
      else if (isRepeatingLinearGradient(backgroundImage2)) this.renderRepeatingLinearGradient(container, backgroundImage2, index);
      else if (isRadialGradient(backgroundImage2)) this.renderRadialGradient(container, backgroundImage2, index);
      if (layerCount > 0) {
  ...
  */
  async renderBackgroundURLImage(container, backgroundImage2, index) {
    ...
    if (image2) {
      ...
      if (!pattern) {
        const resized = this.resizeImage(image2, width, height, container.styles.imageRendering);
  ...
  */
  renderRepeatingLinearGradient(container, backgroundImage2, index) {
    const [path, x, y, width, height] = calculateBackgroundRendering(container, index, [
      null,
      null,
      null
    ...
    const processedStops = processColorStops(backgroundImage2.stops, lineLength || 1);
    ...
    const cacheKey = `rlg|${backgroundImage2.angle}|${Math.round(patternLength)}|${JSON.stringify(backgroundImage2.stops)}`;
    let pattern = this.patternCache.get(cacheKey);
    if (!pattern) {
  ...
  */
  renderLinearGradient(container, backgroundImage2, index) {
    ...
    const [lineLength, x0, x1, y0, y1] = calculateGradientDirection(backgroundImage2.angle, width, height);
    const cacheKey = `lg|${backgroundImage2.angle}|${Math.round(width)}x${Math.round(height)}|${JSON.stringify(backgroundImage2.stops)}`;
    let pattern = this.patternCache.get(cacheKey);
    if (!pattern) {
  ...
  */
  renderRadialGradient(container, backgroundImage2, index) {
    ...
    const position2 = backgroundImage2.position.length === 0 ? [FIFTY_PERCENT] : backgroundImage2.position;
    const x = getAbsoluteValue(position2[0], width);
    const y = getAbsoluteValue(position2[position2.length - 1], height);
    ...
    if (rx > 0 && ry > 0) {
      const cacheKey = `rg|${Math.round(x)}x${Math.round(y)}|${Math.round(rx)}x${Math.round(ry)}|${JSON.stringify(backgroundImage2.stops)}`;
      let pattern = this.patternCache.get(cacheKey);
      if (!pattern) {
        const ownerDocument = this.canvas.ownerDocument ?? document;
        ...
        if (offCtx) {
          ...
          processColorStops(backgroundImage2.stops, offRadius * 2).forEach((s) => gradient.addColorStop(s.stop, asString(s.color)));
...
var BorderImageRenderer = class {
  ...
  renderBorderImage(bounds, image2, slice, repeat, borderTopWidth2, borderRightWidth2, borderBottomWidth2, borderLeftWidth2) {
    const imgW = image2.naturalWidth || image2.width;
    const imgH = image2.naturalHeight || image2.height;
    ...
    if (width <= 0 || height <= 0) return;
    const dT = Math.min(borderTopWidth2, height);
    const dR = Math.min(borderRightWidth2, width);
    const dB = Math.min(borderBottomWidth2, height - dT);
    const dL = Math.min(borderLeftWidth2, width - dR);
    this.drawRegion(image2, 0, 0, sL, sT, left, top, dL, dT);
    this.drawRegion(image2, imgW - sR, 0, sR, sT, left + width - dR, top, dR, dT);
    this.drawRegion(image2, imgW - sR, imgH - sB, sR, sB, left + width - dR, top + height - dB, dR, dB);
    this.drawRegion(image2, 0, imgH - sB, sL, sB, left, top + height - dB, dL, dB);
    const edges = [
      {
        sx: sL,
        sy: 0,
    ...
    for (const edge of edges) {
      ...
      if (edge.repeat === 0) this.ctx.drawImage(image2, edge.sx, edge.sy, edge.sw, edge.sh, edge.dx, edge.dy, edge.dw, edge.dh);
      else if (edge.repeat === 1 || edge.repeat === 2) this.drawRepeatedEdge(image2, edge, isHorizontal, edge.repeat === 2);
    }
    if (slice.fill) {
      const cx = sL;
      const cy = sT;
      ...
      this.drawRegion(image2, cx, cy, cw, ch, tcx, tcy, tcw, tch);
    }
  }
  drawRegion(image2, sx, sy, sw, sh, dx, dy, dw, dh) {
    if (sw > 0 && sh > 0 && dw > 0 && dh > 0) this.ctx.drawImage(image2, sx, sy, sw, sh, dx, dy, dw, dh);
  }
  drawRepeatedEdge(image2, edge, isHorizontal, round) {
    ...
    this.ctx.clip();
    for (let i2 = 0; i2 < tileCount; i2++) {
      const offset = i2 * tileSize;
      const remaining = tgLength - offset;
      const clampedSize = Math.min(tileSize, remaining);
      if (clampedSize <= 0) break;
      if (isHorizontal) this.ctx.drawImage(image2, edge.sx, edge.sy, edge.sw, edge.sh, edge.dx + offset, edge.dy, clampedSize, edge.dh);
      else this.ctx.drawImage(image2, edge.sx, edge.sy, edge.sw, edge.sh, edge.dx, edge.dy + offset, edge.dw, clampedSize);
    }
    this.ctx.restore();
  }
};
  ...
  */
  applyEffects(effects) {
    ...
    for (let i2 = 0; i2 < effects.length; i2++) {
      const effect = effects[i2];
      if (isHeavy(effect)) {
        if (!this.didSave) {
          this.ctx.save();
          this.didSave = true;
        }
        if (i2 === lastHeavyIdx) this.saveAtDepth = this.activeEffects.length;
      }
...
var calculateObjectFitRendering = (intrinsicWidth, intrinsicHeight, box, objectFit2, objectPosition2) => {
  let sx = 0;
  let sy = 0;
  ...
  const posX = objectPosition2 ? getAbsoluteValue(objectPosition2[0], 1) : 0.5;
  const posY = objectPosition2 ? getAbsoluteValue(objectPosition2[1], 1) : 0.5;
  if (objectFit2 === 2) if (imgRatio > boxRatio) {
    dh = dw / imgRatio;
  ...
  else if (objectFit2 === 4) if (imgRatio > boxRatio) {
    sw = sh * boxRatio;
  ...
  } else if (objectFit2 === 16) if ((imgRatio > boxRatio ? dw : dh * imgRatio) < (sw > dw ? sw : dw)) if (imgRatio > boxRatio) {
    dh = dw / imgRatio;
    dy += (box.height - dh) * posY;
  } else {
    dw = dh * imgRatio;
...
async function renderReplacedElements(ctx, context, options, iframeRendererFactory, container, curves, styles, renderReplacedElementFn) {
  if (container instanceof ImageElementContainer) try {
    const image2 = await context.cache.match(container.src);
    ...
    renderReplacedElementFn(container, curves, image2);
...
function renderFormElements(ctx, textRenderer, pathFn, container, styles) {
  ...
  if (isTextInputElement(container) && container.value.length) {
    ...
    const lineHeight2 = computeLineHeight(styles.lineHeight, fontSizeValue);
    lines.forEach((line, index) => {
      ...
      const lineBounds = new Bounds(lineX, textBounds.top + index * lineHeight2, 0, 0);
...
async function renderListMarker(ctx, context, textRenderer, paint, container, styles) {
  ...
  if (container.styles.listStyleImage !== null) {
    ...
    if (img.type === 0) {
      const url = img.url;
      try {
        const image2 = await context.cache.match(url);
        ctx.drawImage(image2, container.bounds.left - (image2.width + 10), container.bounds.top);
      } catch (e) {
...
var CanvasRenderer = class CanvasRenderer2 {
  ...
  */
  ...
  async renderNodeContent(paint) {
    ...
    }, (ctx, opts) => new CanvasRenderer2(ctx, opts), container, curves, styles, (c, cv, img) => this.renderReplacedElement(c, cv, img));
  ...
  async renderNodeBackgroundAndBorders(paint) {
    ...
    if (styles.borderImageSource) {
      ...
      if (source.type === 0) {
        ...
        try {
          const image2 = await this.context.cache.match(url);
          if (image2) {
            const bounds = paint.container.bounds;
            this.borderImageRenderer.renderBorderImage(bounds, image2, styles.borderImageSlice, styles.borderImageRepeat, Math.max(0, styles.borderTopWidth), Math.max(0, styles.borderRightWidth), Math.max(0, styles.borderBottomWidth), Math.max(0, styles.borderLeftWidth));
          }
        } catch (e) {
  ...
  */
  async renderTextClippedBackground(paint) {
    ...
    const writingMode2 = styles.writingMode;
    const letterSpacing2 = styles.letterSpacing;
    for (const textNode of container.textNodes) for (const textBound of textNode.textBounds) {
      ...
      if (letterSpacing2 > 0) this.renderTextMaskWithLetterSpacing(offCtx, textBound, letterSpacing2, localLeft, localTop, writingMode2);
  ...
  */
  renderTextMaskWithLetterSpacing(ctx, text, letterSpacing2, x, y, writingMode2) {
    const letters = segmentGraphemes(text.text);
    let offset = x;
    for (const letter of letters) {
      if (isVerticalWritingMode(writingMode2)) {
      ...
      offset += ctx.measureText(letter).width + letterSpacing2;
    }
  }
};
...
var Cache = class {
  ...
  */
  async preloadAll(concurrency = 10) {
    ...
    for (let i2 = 0; i2 < urls.length; i2 += limit) {
      const operations = urls.slice(i2, i2 + limit).map((url) => this._addImageWithPending(url));
...
var OriginChecker = class {
  constructor(window2) {
    if (!window2 || !window2.document) throw new Error("Valid window object required for OriginChecker");
    if (!window2.location || !window2.location.href) throw new Error("Window object must have valid location");
...
var Context = (_a6 = class {
  constructor(options, windowBounds, config) {
    ...
    this.instanceName = `#${_a6.instanceCount++}`;
    this.config = config;
...
var assembleRenderOptions = (opts, backgroundColor2, left, top, width, height, devicePixelRatio) => ({
  canvas: opts.canvas,
...
var renderElement = async (element, opts, config) => {
  ...
  const renderOptions = assembleRenderOptions(opts, backgroundColor2, left, top, width, height, defaultView.devicePixelRatio ?? 1);
  let canvas;
  let root;
  try {
    if (foreignObjectRendering) {
      ...
      if (backgroundColor2 === root.styles.backgroundColor) root.styles.backgroundColor = COLORS.TRANSPARENT;
